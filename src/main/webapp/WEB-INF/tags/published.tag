...
<div class="reviews_op">
  <div class="reg_text">
    ...
    <fmt:message key="review.published" bundle="${lang}"/>
  </div>
  <div class="reg_text">
    <fmt:setLocale value="${locale}"/>
    <fmt:setBundle basename="i18n" var="lang"/>
  ...
  <c:if test="${not empty review}">
    <form action="${pageContext.request.contextPath}/controller?action=published" method="post">
      ...
      <div class="reg_text">
        <button type="submit">
          <fmt:setLocale value="${locale}"/>
          <fmt:setBundle basename="i18n" var="lang"/>
          <fmt:message key="button.published" bundle="${lang}"/>
        </button>
      </div>
    </form>
  </c:if>
